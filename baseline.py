...
def sub(train, test, best_iter):
    ...
    sub=pd.read_csv("RawData/round1_ijcai_18_test_a_20180301.txt", sep="\s+")
    sub=pd.merge(sub,sub1,on=['instance_id'],how='left')
    sub=sub.fillna(0)
    #sub[['instance_id', 'predicted_score']].to_csv('result/result0320.csv',index=None,sep=' ')
    sub[['instance_id', 'predicted_score']].to_csv('total_data/result0326.txt',sep=" ",index=False)


if __name__ == "__main__":
    train = pd.read_csv("RawData/round1_ijcai_18_train_20180301.txt", sep="\s+")
    test = pd.read_csv("RawData/round1_ijcai_18_test_a_20180301.txt", sep="\s+")
